...
export const motivationalPostSchema = z.object({
  ...
  post: z.object({
    ...
    slides: z.array(z.object({
      ...
      content: z.object({
        ...
        body: z.string().optional().describe("SLIDE 1: Usually empty - use title. SLIDE 2: Main direct manifesto text (80-120 words MAX, up to 140 words if necessary) expanding on slide 1's concept. CRITICAL: Use double line breaks (\\n\\n) to separate each paragraph - DO NOT write one continuous block of text. Write with conversational, direct language: clear statements, actionable insights, bold declarations. Use strategic typography emphasis: bold key words and phrases, italicize important concepts for visual rhythm. Create natural reading flow: mix short punchy sentences with medium ones for varied pace. Write like a direct conversation: clear, uncompromising statements that grab attention. PARAGRAPH STRUCTURE: Use short paragraphs (2-3 sentences max) for easy scanning and digestion. CONVERSATIONAL TONE: Use direct, imperative language ('Do the hard thing.' 'Stop fucking delaying.') STRATEGIC EMPHASIS: Bold + italicize key words throughout the text for visual texture and emphasis. SENTENCE VARIETY: Mix short punchy lines ('Because hard things are hard.') with medium explanatory ones. CRITICAL: Create 3-5 short paragraphs, each 2-3 sentences maximum. Use strategic single-line paragraphs in the MIDDLE for dramatic impact and rhythm. Create natural pauses with paragraph breaks - make it digestible on fast scroll. Reference: 1 line ≈ 10 words. Avoid any paragraph exceeding 3 sentences to prevent dense blocks. CRITICAL FINAL REQUIREMENT: ALWAYS end with a single-line paragraph (maximum 10 words) that serves as your most powerful statement. EXAMPLE FORMAT: 'First short paragraph here.\\n\\nSecond paragraph with emphasis.\\n\\nSingle line impact.\\n\\nFinal truth.'")
      }),
  ...
  caption: z.string().describe("Instagram caption that feels like a natural extension of the post's direct message. Should be raw, authentic, and actionable - like the user's own reflection on the content. Avoid motivational clichés, generic language, or performative tone. Instead, write as if the user is sharing their genuine thoughts about the post's message. Can be a question, observation, or personal insight that invites action. Keep it concise (1-2 sentences max) and maintain the same direct, uncompromising voice as the post itself."),
